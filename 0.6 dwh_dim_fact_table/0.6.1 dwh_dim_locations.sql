...
IF OBJECT_ID('[dwh_computer_sales].[dbo].[dim_locations]', 'U') IS NULL
BEGIN
    CREATE TABLE [dwh_computer_sales].[dbo].[dim_locations] (
...
TRUNCATE TABLE [dwh_computer_sales].[dbo].[dim_locations];
GO

-- Load data into clean dim_locations from staging raw
INSERT INTO [dwh_computer_sales].[dbo].[dim_locations] (continent, country_or_state, province_or_city)
SELECT DISTINCT
    r.[Continent],
    r.[Country_or_State],
    r.[Province_or_City]
...
WHERE r.[Continent] IS NOT NULL
  ...
  AND NOT EXISTS (
      SELECT 1
      FROM [dwh_computer_sales].[dbo].[dim_locations] AS d
...
--DROP TABLE IF EXISTS [dwh_computer_sales].[dbo].[dim_locations]
SELECT * FROM [dwh_computer_sales].[dbo].[dim_locations]
GO