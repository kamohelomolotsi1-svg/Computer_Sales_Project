-- 0.5.3 Clean and load dim_pc_product into clean_computer_sales
...
IF OBJECT_ID('[clean_computer_sales].[dbo].[dim_pc_product]', 'U') IS NULL
BEGIN
    CREATE TABLE [clean_computer_sales].[dbo].[dim_pc_product] (
...
TRUNCATE TABLE [clean_computer_sales].[dbo].[dim_pc_product];
GO

-- Load data into clean dim_pc_product from staging raw
INSERT INTO [clean_computer_sales].[dbo].[dim_pc_product] (pc_make, pc_model, storage_type, ram, storage_capacity)
SELECT DISTINCT
    r.[PC_Make],
    r.[PC_Model],
    r.[Storage_Type],
...
WHERE r.[PC_Make] IS NOT NULL
  ...
  AND NOT EXISTS (
      SELECT 1
      FROM [clean_computer_sales].[dbo].[dim_pc_product] AS d
...
SELECT * FROM [clean_computer_sales].[dbo].[dim_pc_product];
GO
